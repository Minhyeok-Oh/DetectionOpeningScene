...
for i in range(GROUPING_VIDEO_NUMBER):
    hash_A = []
    ...
    for j in range(GROUPING_VIDEO_NUMBER):
        if i == j:
            continue
        hash_B = []
    ...
    for k in range(1, int(FRAME_NUMBER) + 1):
        count = 0
        for m in range(GROUPING_VIDEO_NUMBER - 1):
...
def group_compare_to_get_hash(filename):
    ...
    for j in range(GROUPING_VIDEO_NUMBER - 1):
        hash_BB = []
...
filename_one = (str(DATASETFOLDER) + '/' +'사이코지만 괜찮아 E09.200705.1080p.WEB-DL.x264.AAC-Deresisi.mp4')
